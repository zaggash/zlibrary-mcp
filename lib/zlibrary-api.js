...
async function downloadBookToFile(id, format = null, outputDir = './downloads') {
  try {
    ...
    const downloadInfo = await getDownloadLink(id, format);
    
    if (!downloadInfo || downloadInfo.error) {
      throw new Error(downloadInfo.error || 'Failed to get download link');
    }
    ...
    // Download the file
    await downloadFile(downloadInfo.download_link, outputPath);
