...
const REQUIRED_PACKAGE = 'zlibrary';
...
async function getCacheDir() {
    // Dynamically import env-paths if not already loaded
    if (!envPaths) {
...
async function installDependencies(venvPythonPath) {
    console.log(`Installing ${REQUIRED_PACKAGE} package using ${venvPythonPath}...`);
    try {
        const { stderr, code } = await runCommand(venvPythonPath, ['-m', 'pip', 'install', REQUIRED_PACKAGE]);
        if (code === 0) {
            console.log(`${REQUIRED_PACKAGE} installed successfully.`);
        } else {
            throw new Error(`Failed to install ${REQUIRED_PACKAGE} (exit code ${code}). Stderr: ${stderr}`);
        }
    } catch (error) {
        // Catch spawn errors or re-throw non-zero exit code errors
...
async function checkPackageInstalled(venvPythonPath) {
    console.log(`Checking if ${REQUIRED_PACKAGE} is installed in ${venvPythonPath}...`);
    try {
        // Don't log output for this check unless debugging
        const { code } = await runCommand(venvPythonPath, ['-m', 'pip', 'show', REQUIRED_PACKAGE], {}, false);
        // pip show returns 0 if found, 1 if not found
        if (code === 0) {
            console.log(`${REQUIRED_PACKAGE} is installed.`);
            return true;
        } else {
            console.log(`${REQUIRED_PACKAGE} not found (pip show exit code ${code}).`);
            return false;
        }
    } catch (error) {
...
async function ensureVenvReady() {
    ...
    if (venvPythonPath) {
        ...
        if (isInstalled) {
            ...
            console.log(`${REQUIRED_PACKAGE} not found in configured venv. Re-installing...`);
            try {
                await installDependencies(venvPythonPath);
                console.log('Venv re-validated successfully.');
                return; // Setup complete after re-install
            } catch (installError) {
                console.error(`Failed to reinstall ${REQUIRED_PACKAGE}. Attempting full venv recreation.`);
